    ...
    while True:
        ...
        for block_index in range(1, len(blocks)):
            ...
            for line_index in range(1, len(block)):
                destination_range_start, source_range_start, range_length = int(block[line_index].split(
                )[0]), int(block[line_index].split()[1]), int(block[line_index].split()[2])
                if key >= source_range_start and key < source_range_start + range_length:
                    ...
                    neutral_slack = min(
                        neutral_slack, key - source_range_start - range_length + 1)
