...
cords = {tuple(current)}
for line in lines:
    relevant_info = line.split('#')[1][:-1]
    dir = hex_to_dir[relevant_info[-1]]
    steps = int(relevant_info[:-1], 16)
...
for y_index in range(len(all_y)-1):
    ...
    # compute area
    #int() is needed to prevent numpy scalar overflow
    area = int(width_between) * int(hight_between) + int(width_at_y)
    ans += area
