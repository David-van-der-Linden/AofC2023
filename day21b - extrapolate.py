...
def compute_ans(lines, max_steps):
    max_steps = max_steps
    ...
    def compute_diagonal(max_steps, vertical_len_chunk, horizontal_len_chunk, leave_center_dist, max_dist_in_chunk, relevant_chunk, max_reachable_even, max_reachable_odd):
        ...
        for chunk_i in tqdm(range((max_steps // vertical_len_chunk) + 2)):
            # consider
            # starting_dist = max_steps - max_dist_in_chunk
            # starting_dist = leave_center_dist + chunk_i * vertical_len_chunk + chunk_j * horizontal_len_chunk
            # rewriting yields
            # chunk_j * horizontal_len_chunk =  max_steps - max_dist_in_chunk - (leave_center_dist + chunk_i * vertical_len_chunk)
            # chunk_j = (max_steps - max_dist_in_chunk - (leave_center_dist + chunk_i * vertical_len_chunk)) // horizontal_len_chunk
            ...
            range_start = (((max_steps - max_dist_in_chunk - (leave_center_dist +
                                                              chunk_i * vertical_len_chunk)) // horizontal_len_chunk) // 2) * 2
            ...
            for chunk_j in range(range_start, range_end):
                ...
                elif starting_dist <= max_steps - max_dist_in_chunk:
...
# print('ans', compute_ans(lines, 26501365))
max_steps = 26501365
rest = max_steps % (len(lines)*2)
xs = [rest + len(lines)*2 * i for i in range(4)]
ys = [compute_ans(lines, x) for x in xs]

# first method
poly = np.polyfit(xs, ys, deg=2)
y_int = np.polyval(poly, max_steps)
print('first method ans', y_int)
...
for i in range(len(xs)):
    system = system + \
        (Eq(ys[i], a * xs[i] ** 2 + b * xs[i] + c),)


solutions = solve(system, (a, b, c))
print("Solutions:")
print(solutions)
for solution in solutions:
    print(solution)
# ans = 15287 * max_steps ** 2 + 28518 * max_steps + 40469
#     # ans = solution[0] * max_steps ** 2 + solution[1] * max_steps + solution[2]
# print('ans', ans)