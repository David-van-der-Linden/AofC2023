...
# print(loop_cells)
big_grid = [['.' for _ in range(2*len(lines[0])+1)]
...
def label_a_and_almost_b(cell_a, cell_b):
    ...
    big_grid[int(old_to_new((cell_a[0] + cell_b[0]) / 2))
             ][int(old_to_new((cell_a[1] + cell_b[1]) / 2))] = '#'
...
class Board:
    ...
    def color_this_and_touching(self, initial_loc: tuple[int], symbol: str):
        queue = [initial_loc]
        visited = [[False for _ in range(len(self.grid[0]))]
