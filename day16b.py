...
# idea b do it for all possible starting positions
...
class laser_board():
    def __init__(self, lines, starting_beam):
        self.lines = lines
...
for i in range(len(lines)):
    ...
    starting_beams.append((i, len(lines[0])-1, 'L'))
for j in range(len(lines[0])):
    starting_beams.append((0, j, 'D'))
    starting_beams.append((len(lines)-1, j, 'U'))

ans = 0
for starting_beam in tqdm(starting_beams):
    obj = laser_board(lines, starting_beam)
    while len(obj.beams) > 0:
        obj.move()

    energized_in_this_config = 0
    for i in range(len(obj.energized)):
        for j in range(len(obj.energized[0])):
            if obj.energized[i][j]:
                energized_in_this_config += 1
    ans = max(ans, energized_in_this_config)
